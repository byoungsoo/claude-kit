...
```bash
pip3 install -r ppt-generator/skills/ppt-generator/requirements.txt --break-system-packages
...
```
1. @ppt-generator-research       — 주제 리서치 (웹 검색, 통계, 시각화 포인트 수집)
2. @ppt-generator-outline        — 내러티브 구조 설계 (슬라이드 목록, 전환 흐름)
3. @ppt-generator-content        — 슬라이드별 콘텐츠 생성 (텍스트, 차트 스펙, 다이어그램)
4. @ppt-generator-design         — 레이아웃·디자인 스펙 결정
5. @ppt-generator-qa             — 품질 검토 (7점 미만 슬라이드 자동 재생성, 최대 2회)
...
```bash
cd ppt-generator/skills/ppt-generator
