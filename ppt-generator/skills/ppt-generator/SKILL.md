...
`[1/5] 리서치 중...` 을 사용자에게 알린 뒤, `@__KIT_NAME__-research` 에이전트를 **백그라운드로** 실행합니다:
...
`[2/5] 슬라이드 구조 설계 중...` 을 사용자에게 알린 뒤, `@__KIT_NAME__-outline` 에이전트를 **백그라운드로** 실행합니다:
...
`[3/5] 슬라이드 콘텐츠 작성 중...` 을 사용자에게 알린 뒤, `@__KIT_NAME__-content` 에이전트를 **백그라운드로** 실행합니다:
...
- 사용자가 **A** 선택 → `@__KIT_NAME__-aws-architect` 에이전트 실행
- 사용자가 **B** 선택 또는 응답 없이 건너뜀 → 해당 슬라이드의 `diagram.aws_diagram`을 `null`로 설정하고 다음 단계 진행

#### 다이어그램 생성 (A 선택 시)

`[3.5/5] AWS 아키텍처 다이어그램 생성 중...` 을 사용자에게 알린 뒤, `@__KIT_NAME__-aws-architect` 에이전트를 **백그라운드로** 실행합니다:
...
`[4/5] 디자인 스펙 생성 중...` 을 사용자에게 알린 뒤, `@__KIT_NAME__-design` 에이전트를 **백그라운드로** 실행합니다:
...
`[5/5] 품질 검토 중...` 을 사용자에게 알린 뒤, `@__KIT_NAME__-qa` 에이전트를 **백그라운드로** 실행합니다:
...
- `[재작업] {N}개 슬라이드 개선 중...` 을 사용자에게 알린 뒤 `@__KIT_NAME__-content`를 **백그라운드로** 재실행
  ```
  다음 슬라이드를 수정하세요:
