...
SKILL.md (오케스트레이터)
    ↓
  @__KIT_NAME__-research  — 웹 리서치 → research JSON
  @__KIT_NAME__-outline   — 내러티브 구조 → outline JSON
  @__KIT_NAME__-content   — 슬라이드 콘텐츠 → slides JSON
  @__KIT_NAME__-design    — 레이아웃·테마 → design JSON
  @__KIT_NAME__-qa        — 품질 검토 → 7점 미만 재생성 (최대 2회)
    ↓
...
| 파일 | 역할 |
|------|------|
| `skills/ppt-generator/SKILL.md` | 파이프라인 오케스트레이터 |
...
| `skills/ppt-generator/src/ppt_generator/cli/render.py` | JSON → PPTX 렌더러 CLI |
| `skills/ppt-generator/src/ppt_generator/rendering/` | 슬라이드 렌더링 모듈 |
| `skills/ppt-generator/src/ppt_generator/schema/` | Pydantic 데이터 계약 |
| `skills/ppt-generator/themes/` | 디자인 테마 JSON |
| `skills/ppt-generator/scripts/` | PPTX 편집 유틸리티 |
