...
- **source/target 명시 원칙**: 모든 edge는 반드시 `source="cellId"` `target="cellId"` 속성을 명시한다. floating 연결(source/target 없이 좌표만 지정) 금지.
- **연결 추가 금지**: 스펙의 연결 표에 없는 edge를 임의로 추가하지 않는다. 스펙에 없으면 그리지 않는다. 응답 경로·egress 경로·관리 서비스 연결을 보완해 넣는 것을 금지한다.

**연결선 라우팅 규칙 (아이콘 관통 금지)**:

edge가 source/target 이외의 다른 아이콘이나 그룹 레이블 위를 지나가면 안 된다.

1. **인접 노드 연결** — source와 target 사이에 다른 노드가 없는 경우:
   연결점을 지정하지 않고 draw.io 자동 라우팅에 맡긴다. (exitX/entryX 생략)

2. **스킵 연결** — source와 target 사이(직선 경로상)에 다른 노드가 놓인 경우:
   자동 라우팅은 중간 노드를 관통하므로, **반드시** 아래를 모두 지정한다.
   - `exitX/exitY/exitDx=0/exitDy=0` 과 `entryX/entryY` 로 우회 방향의 변(측면)에서 나가고 들어오게 한다
   - `<Array as="points">` 에 우회 경로 waypoint를 넣어 라우팅 거터를 통과시킨다

   **라우팅 거터**: 아이콘 열/행 바깥으로 40px 떨어진 가상의 통로.
   - top-down 레이아웃: 아이콘 열의 좌측 또는 우측 (해당 그룹 여백 60px 안쪽)
   - left-right 레이아웃: 아이콘 행의 상단 또는 하단

   ```xml
   <!-- A(위) → C(아래), 사이에 B가 있는 경우: 우측 거터로 우회 -->
   <mxCell id="e_a_c" value="" style="edgeStyle=orthogonalEdgeStyle;rounded=0;curved=0;exitX=1;exitY=0.5;exitDx=0;exitDy=0;entryX=1;entryY=0.5;entryDx=0;entryDy=0;" edge="1" parent="1" source="a" target="c">
     <mxGeometry relative="1" as="geometry">
       <Array as="points">
         <mxPoint x="{아이콘열_우측_x + 40}" y="{a의 중심 y}"/>
         <mxPoint x="{아이콘열_우측_x + 40}" y="{c의 중심 y}"/>
       </Array>
     </mxGeometry>
   </mxCell>
   ```

   waypoint 좌표는 부모 셀 기준 상대좌표가 아니라 **다이어그램 절대좌표**로 계산한다.
   여러 스킵 연결이 있으면 거터 오프셋을 40 / 70 / 100 … 으로 어긋나게 해 선끼리 겹치지 않게 한다.

3. **XML 작성 후 자체 검사**: 각 edge마다 source 중심 → target 중심 경로가 다른 노드의 사각형(x, y, x+70, y+70)과 교차하는지 확인한다. 교차하면 규칙 2를 적용해 우회시킨다.

**그룹 크기 동적 계산 원칙**:
그룹 크기는 내부 자식 요소를 모두 감싸는 최소 크기 + 여백으로 계산한다. 고정값 사용 금지.

아이콘 크기: 70, 간격: 80, 그룹 내 여백: 60, 레이블 높이: 30

**Subnet (중첩 그룹 없을 때)**:
- 너비 = (아이콘 수 × (70 + 80)) - 80 + 60×2
- 높이 = 30 (레이블) + 60 (상단 여백) + 70 (아이콘) + 60 (하단 여백) = 220

**Subnet (ASG 등 중첩 그룹 있을 때)**:
- 중첩 그룹 너비 = (내부 아이콘 수 × (70 + 80)) - 80 + 60×2
- 중첩 그룹 높이 = 30 + 60 + 70 + 60 = 220
...
- Subnet 너비 = (1 × 150) - 80 + 120 = 190, 높이 = 220
- AZ 너비 = 190 + 120 = 310, 높이 = 220×2 + 60 + 30 + 120 = 650
- VPC 너비 = (2 × (310+60)) - 60 + 120 = 800, 높이 = 650 + 30 + 120 = 800
